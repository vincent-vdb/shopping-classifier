...
base_data_path = "."
output_path = "./save-big-12-FCN-nofc"
...
def y_to_classes(y):
	return [classes[int(i)] for i in np.where(y == 1.)]
...
class DSLoader:
	...
	def reshuffle(self):
		...
		self.index_dir[:self.n_epoch] = [self.index_dir[i] for i in order]
		self.index_img[:self.n_epoch] = [self.index_img[i] for i in order]
		#self.nimages, self.index_dir, self.index_img = build_random_indices(self.dataset)
		#np.random.shuffle(self.indices)
...
def load_network(weightsfile,exclusive=False):
	net = cnn_new.make_fcn(nclasses=len(classes),dropout=True,no_fc=True,loadweights=False,exclusive=exclusive)
	net.load_weights(weightsfile)
	return net

def eval_image(imagefile,weights=default_trained_weights):
...
def train_net(load_weights=None,valid_pct=0.1,n_epoch=100,batch_size=10,base_lr=1e-4,ds_reduction_factor=1.,patience=1,reload_if_bad=True,increase_if_good=0.05,initial_weights_path=vgg16_path,output_logfile=logfile, output_weights=weightspath,catcross=True):
	model = cnn_new.make_fcn(nclasses=len(classes),dropout=True,no_fc=True,exclusive=catcross)
	if(load_weights):
